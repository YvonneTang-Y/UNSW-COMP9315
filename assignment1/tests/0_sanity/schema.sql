...
create or replace function dms_string(coord text)
returns text 
as $$
DECLARE
    gc_string text;
...
BEGIN
    ...
    if coordvalues[1] ~ '^[\d.°''"]*[NS]' then
        result := result || ',' || coordvalues[1] || ',' || coordvalues[2];
    else 
        result := result || ',' || coordvalues[2] || ',' || coordvalues[1];
    end if;
    return result;
